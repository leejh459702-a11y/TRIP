...
describe('computeDensity', () => {
  ...
  it('빈 하루는 여유(level 1)로 판정한다', () => {
    const result = computeDensity({ totalTravelMin: 0, totalStayMin: 0, placeCount: 0 }, dayStart);
  ...
  it('이동 비중이 높고 장소가 많으면 레벨이 올라간다', () => {
    const light = computeDensity(
      { totalTravelMin: 10, totalStayMin: 200, placeCount: 2 },
      dayStart,
    );
    const heavy = computeDensity(
      { totalTravelMin: 180, totalStayMin: 200, placeCount: 8 },
      dayStart,
    );
  ...
  it('여유분이 음수(하루 예산 초과)면 0.2가 가산된다', () => {
    ...
    const overBudget = computeDensity(
      { totalTravelMin: 100, totalStayMin: 900, placeCount: 1 },
      dayStart,
    );
    const underBudget = computeDensity(
      { totalTravelMin: 100, totalStayMin: 100, placeCount: 1 },
      dayStart,
    );
